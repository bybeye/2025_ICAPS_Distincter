...
void SearchStatistics::print_basic_statistics() const {
    ...
    log << ", model time: " << cumulate_model_time << "s, state num: " << total_state;
    if (total_action > 0) {
	log << ", action num: " << total_action <<  ", action time: " << cumulate_sym_time << "s";
    }
...
void SearchStatistics::print_detailed_statistics() const {
    ...
    log << "Cumulate heuristic function time: " << cumulate_heuristic_time << "s." << endl;
    log << "Cumulate data time: " << cumulate_data_time << "s." << endl;
    log << "Cumulate model time: " << cumulate_model_time << "s." << endl;
    log << "Cumulate sym time: " << cumulate_sym_time << "s." << endl;
    log << "Action num: " << total_action << "." << endl;
    log << "Action invoke times: " << action_invoke_times << "." << endl;
    log << "State num: " << total_state << "." << endl;
