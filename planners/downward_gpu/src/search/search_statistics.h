...
class SearchStatistics {
    ...
    double cumulate_data_time;
    double cumulate_model_time;
    double cumulate_sym_time;
    double cumulate_heuristic_time;
    int model_invoke_times;
    int total_action = 0;
    int total_state = 0;
    int action_invoke_times = 0;
...
public:
    ...
    void inc_total_action(int inc = 1) { total_action += inc; }
    void inc_total_state(int inc = 1) { total_state += inc; }
    void inc_action_invoke_times(int inc = 1) { action_invoke_times += inc; }

    void update_sym_time(double sym_time) {cumulate_sym_time = sym_time;}
    void update_heuristic_time(double cost_time) {cumulate_heuristic_time = cost_time;}
    void update_data_time(double cost_time) {cumulate_data_time = cost_time;}
    void update_model_time(double cost_time) { cumulate_model_time = cost_time; }
    void update_model_invoke_times(int times) { model_invoke_times = times; }
    ...
    double get_data_time() const {return cumulate_data_time; }
    double get_model_time() const { return cumulate_model_time; }
    double get_heuristic_time() const { return cumulate_heuristic_time; }
    double get_sym_time() const { return cumulate_sym_time; }
    int get_model_invoke_times() const { return model_invoke_times; }
    int get_action_invoke_times() const { return action_invoke_times; }
    /*
