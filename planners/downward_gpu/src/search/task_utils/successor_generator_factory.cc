...
public:
    explicit OperatorGrouper(const vector<OperatorInfo> &operator_infos, int depth, GroupOperatorsBy group_by, OperatorRange range)
      : operator_infos(operator_infos), depth(depth), group_by(group_by), range(range) {
    }
...
SuccessorGeneratorFactory::SuccessorGeneratorFactory(const TaskProxy &task_proxy) : task_proxy(task_proxy) {
}
...
GeneratorPtr SuccessorGeneratorFactory::construct_fork(
    ...
    int size = nodes.size();
    if (size == 1) {
        // std::cout << "construct_fork 1 " << std::endl;
	// batch_eager_search
        return move(nodes.at(0));
    } else if (size == 2) {
	// std::cout << "construct_fork 2 " << std::endl;        
	...
	// std::cout << "construct_fork other " << size << std::endl;        
        return utils::make_unique_ptr<GeneratorForkMulti>(move(nodes));
    }
}

GeneratorPtr SuccessorGeneratorFactory::construct_leaf(OperatorRange range) const {
    assert(!range.empty());
    ...
    if (operators.size() == 1) {
	// batch_eager_search        
	// std::cout << "GeneratorLeafSingle" << std::endl;
        return utils::make_unique_ptr<GeneratorLeafSingle>(operators.front());
    } else {
	// std::cout << "GeneratorLeafVector" << std::endl;        
...
GeneratorPtr SuccessorGeneratorFactory::construct_switch(
    ...
    if (hash_bytes < vector_bytes) {
        ...
        // std::cout << "GeneratorSwitchHash" << std::endl;
	...
	// std::cout << "GeneratorSwitchVector" << std::endl;
        return utils::make_unique_ptr<GeneratorSwitchVector>(switch_var_id, move(generator_by_value));
    }
}

GeneratorPtr SuccessorGeneratorFactory::construct_recursive(int depth, OperatorRange range) const {
    ...
    while (!grouper_by_var.done()) {
        ...
        if (var == -1) {
            ...
            OperatorGrouper grouper_by_value(operator_infos, depth, GroupOperatorsBy::VALUE, var_range);
