...
protected:
    ...
    */
    ...
    virtual float compute_heuristic(const State &ancestor_state) = 0;

    virtual std::vector<float> compute_heuristic_batch(const std::vector<State> &states) = 0;
...
public:
    ...
    virtual std::vector<float> compute_result_batch(const std::vector<State> &states) override;
