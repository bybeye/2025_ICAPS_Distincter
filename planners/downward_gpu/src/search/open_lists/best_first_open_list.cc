...
protected:
    ...
    void do_insertion(float h, const Entry &entry);
...
template<class Entry>
BestFirstOpenList<Entry>::BestFirstOpenList(const shared_ptr<Evaluator> &evaluator, bool preferred_only)
...
template<class Entry>
void BestFirstOpenList<Entry>::do_insertion(EvaluationContext &eval_context, const Entry &entry) {
    float key = eval_context.get_evaluator_value(evaluator.get());
    buckets[key].push_back(entry);
    ++size;
}

template<class Entry>
void BestFirstOpenList<Entry>::do_insertion(float h, const Entry &entry) {
    float key = h;
    // original, FIFO
    buckets[key].push_back(entry);

    // new, LIFO Last-In-First-Out 
